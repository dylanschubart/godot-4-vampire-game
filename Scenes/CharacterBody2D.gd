...
func _physics_process(delta):
	...
	if (!jumping and velocity.y < 0):
		...
		else: _animated_sprite.play("Jump_Start_Standstill"); return;
	if (jumping and velocity.y > 0): 
		if (_animated_sprite.animation == 'Jump_Start_Moving'): _animated_sprite.play("Jump_End_Moving"); jumping = true; return;
		else: _animated_sprite.play("Jump_End_Standstill"); return;
	#end jumping
